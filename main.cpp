...
struct ieee80211_header {
    /*always 24 bytes*/
    uint8_t    type;//htons(type) == 4(Probe Request), 5(Probe Response), 8(Beacon)
...
int main(int argc, char *argv[]){
    ...
    while(true){
        ...
        //int get_version = (Bf->type & (0x03));
        int get_subtype = (Bf->type>>4);

        packet += 24;
        packet_size -= 24;
        if(get_type == 2 && ((get_subtype & 0x04) == 0)){//10(data frame) 00(version) DATA FRAME
            packet += 8;
            packet_size -= 8;
            ++num_data;
            set_BSSID();
        ...
        else if(get_type == 2 && ((get_subtype & 0x04) != 0)){//10(data frame) 00(version) DATA FRAME
            packet += 8;
            packet_size -= 8;
            ++num_data;
            set_BSSID();
        ...
        else if(Bf->type == (0x80)){//beacon frame
            packet += 12;//fixed parameters!
            packet_size -= 12;
            //after that, ssid parameter(0) always appears!
            ++num_beacon;
            int flag=0;
            set_BSSID();
        ...
        else if(Bf->type == (0x40)){//probe req
            ++num_probe;
            set_BSSID();
            set_STATION();
            int flag=0;
